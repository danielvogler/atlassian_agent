...
# creates uses that entry as its notes. A version whose notes are checked from
# one place and rendered from another is a version whose notes drift.
#
...
# the block of `[0.1.0]: https://...` link definitions at the foot of the file.
...
# Trim the blank lines that the heading and the next one leave behind.
